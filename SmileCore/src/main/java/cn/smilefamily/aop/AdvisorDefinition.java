...
import cn.smilefamily.annotation.aop.match.*;
...
public class AdvisorDefinition implements Comparable<AdvisorDefinition> {
    ...
    private AdvisorDefinition(Optional<Order> optionalOrder, AdviceType adviceType, Method m, BeanDefinition bd) {
    ...
    public static List<AdvisorDefinition> buildFrom(Class<?> aspectClass, BeanDefinition bd) {
    ...
    private static List<AdvisorDefinition> getAdvisorDefinitions(Class<?> aspectClass, Optional<Order> optionalOrder, AdviceType adviceType, BeanDefinition bd) {
    ...
    public boolean equals(Object o) {
        ...
        AdvisorDefinition that = (AdvisorDefinition) o;
        return adviceMethod.equals(that.adviceMethod) && aspectBeanDefinition.equals(that.aspectBeanDefinition);
    }

    @Override
    public int hashCode() {
        return Objects.hash(adviceMethod, aspectBeanDefinition);
    }
    ...
    public Object[] constructArgs(BeanDefinition bd, Object target, Object self, Method targetMethod, Method proceed, Object[] callArgs, Object result, Throwable e) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (param.isAnnotationPresent(Annotated.class)) {
                if (!Annotation.class.isAssignableFrom(param.getType())) {
                    throw new AspectInjectException(param + " of " + adviceMethod + " cannot match the @Annotated type");
                }
                Annotation annotation = targetMethod.getAnnotation((Class<? extends Annotation>) param.getType());
                checkTypeMatch(param, annotation, param + " of " + adviceMethod + " cannot match the @Annotated type " + annotation);
                arguments[i] = annotation;
                continue;
            }
