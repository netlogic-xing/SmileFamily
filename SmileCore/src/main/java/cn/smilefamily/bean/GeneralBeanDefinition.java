...
public class GeneralBeanDefinition implements BeanDefinition {
    ...
    public GeneralBeanDefinition(Context context, String source, String name, Class<?> clazz) {
     ...
     */
    public GeneralBeanDefinition(Context context, String source, String name, Class<?> clazz, String scope, Export export,
        ...
        if (factory == null) {
            Arrays.stream(this.type.getDeclaredConstructors())
                    ...
                    .ifPresent(c -> {//如果有标注为@Factory的有参构造函数，则采用此构造函数生成bean
                        ...
                        factory = () -> BeanUtils.newInstance(c, deps.stream().map(p -> p.getDepValue(context)).toArray());
                    });
        }
        ...
        if (factory == null) {//constructor over static factory method
            Arrays.stream(this.type.getDeclaredMethods())
                    ...
                    .ifPresent(f -> {//采用@Factory静态工厂方法生成实例
                        ...
                        dependencies.addAll(deps);
                        factory = () -> BeanUtils.invokeStatic(f, deps.stream().map(p -> p.getDepValue(context)).toArray());
                    });
        }
